...
bool canCalc(long long& value, std::list<long long> nums)
{
    ...
    msg(leftNum, rightNum, std::atoll((std::to_string(leftNum) + std::to_string(rightNum)).c_str()));
    newNums2.push_front(std::atoll((std::to_string(leftNum) + std::to_string(rightNum)).c_str()));
